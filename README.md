...
4. **"Importar CSV o PDF" button (Economía, being trialed)**: `POST /api/import-csv`. The user uploads their bank's statement as-is — a CSV export or a PDF statement, from any bank — and it's sent to Gemini (`GEMINI_API_KEY`, the same key already used by `/api/quick-transaction` for AI category inference — no new key needed; model overridable via `GEMINI_CSV_MODEL`, defaults to `gemini-3.6-flash`), which detects the format and returns normalized transactions (ISO date, signed amount, one of the app's 9 fixed categories) via structured JSON output. A CSV's raw text goes straight into the prompt; a PDF is sent as a native `inlineData` document part (Gemini 2.5 reads PDFs directly, tables included, scanned or text-based) instead of extracting text server-side — the client (`economy-section.tsx`) picks which by file extension/MIME type and base64-encodes PDFs before sending. The prompt explicitly tells the model to skip rows that are internal money movement rather than a real expense/income (currency-pocket conversions, transfers between the user's own accounts) — real statements (Revolut especially) mix these in with real transactions. Gemini also reports a per-row ISO 4217 currency code whenever the statement distinguishes one (a multi-currency file like Revolut's); the client then converts each such row to the home currency via `lib/exchange-rates.ts` (same daily-cached lookup the manual add-transaction flow uses) before inserting, keeping the original amount/currency on the transaction — a row whose currency has no rate available that day is left out of the import rather than guessed. The client then drops anything that already matches an existing transaction (same date + amount + description) before inserting the rest via the same batch-insert path as `addTransactions` — so re-uploading the same statement, or one with overlapping dates, doesn't duplicate rows. No preview/review screen — it imports directly. On failure, the error shown includes Gemini's actual status/reason (quota, retired model, truncated response, etc.) instead of a generic message, so a real-world failure is diagnosable from the on-screen toast alone. Size caps: 300,000 characters for CSV text, 20MB of base64 (~15MB PDF) for PDFs — comfortably more than a normal statement. See the note at the top of `app/api/import-csv/route.ts` for the privacy trade-off this implies, and the Privacy note below.
...
**Privacy note**: Claude does not read or analyze the actual contents of users' transactions/income data while building and maintaining this app — the code is written and edited without looking at real rows from Supabase. The one deliberate exception is the CSV import feature above: there, the literal content of whatever bank statement the user uploads (dates, amounts, descriptions) is sent to the Gemini API on each import, because that's the only practical way to support "any bank's CSV" without hand-writing a parser per bank. That trade-off was discussed and chosen explicitly (see `app/api/import-csv/route.ts`) — everywhere else in the app, financial data never leaves Supabase/Vercel. This same information, written for end users rather than for whoever maintains the code, now also lives in the app itself at `/privacy` (linked from the bottom of Ajustes), alongside a `/terms` page — both static, Spanish-only pages (`app/privacy/page.tsx`, `app/terms/page.tsx`).
...
### 11 Sep 2026
- **Presupuestos (category budgets) removed entirely**: the feature never got real usage, so it was pulled to keep the app simpler — `CategoryBudget` type, `CategoryBudgetRow`, `fetchBudgets`/`rowToBudget`/`setBudget` in the store, and the whole budgets card in Resumen (`overview-section.tsx`) are gone. `AppData` no longer has a `budgets` field.
- **"Importar CSV" button added to Economía (being trialed)** — see point 4 under "Bank expense import" above. Uses Gemini (already-configured `GEMINI_API_KEY`) to read any bank's CSV export and normalize it, with client-side dedup against existing transactions. No preview screen yet — imports directly, per an explicit, discussed privacy trade-off (see the Privacy note above).
- **PDF support added to the same import feature** — the button now reads "Importar CSV o PDF"; a PDF statement is sent to Gemini as a native document instead of a CSV's raw text, reusing the same schema/validation. See point 4 above.
- **Import diagnostics improved and prompt hardened after a real-world test failed**: a real Revolut export (multi-currency, with internal currency-exchange rows) failed to import with a generic "can't read it with AI" message. The route now (a) retries once on a transient Gemini error (429/5xx) before giving up, (b) surfaces Gemini's actual status/reason in the on-screen error instead of a generic string, and (c) explicitly tells the model to skip internal transfers/currency conversions instead of importing them as real transactions.
- **In-app Privacy Policy and Terms of Use pages** (`/privacy`, `/terms`), linked from the bottom of Ajustes. Static, Spanish-only pages spelling out for end users what the README's Privacy note already said for whoever maintains the code — including the CSV-import exception.
- **Gemini model updated after a real 404 in production**: `gemini-2.5-flash` (used by `/api/import-csv`) was retired by Google without much notice — a real CSV import failed with `404 "This model ... is no longer available"`. Switched the default to `gemini-3.6-flash` (Google's own suggested replacement), and proactively moved `/api/quick-transaction`'s category-inference model from `gemini-2.5-flash-lite` to `gemini-3.5-flash-lite` before it hit the same wall — that one now also reads from an env var (`GEMINI_QUICK_MODEL`) instead of being hardcoded, so the next retirement doesn't need a code change.
- **Gemini "thinking" turned down after that model swap caused timeouts**: unlike the 2.5 family, Gemini's 3.x models think before responding by default (`medium` level) — with a real, larger CSV this pushed past the 55s budget `/api/import-csv` gave Gemini, aborting with "The operation was aborted due to timeout". Both Gemini calls (`import-csv` and `quick-transaction`) now request the lowest available thinking level for 3.x models (`thinkingLevel: "low"`, since the Flash family can't disable it entirely) — plain extraction/classification doesn't need deep reasoning. `quick-transaction`'s timeout budget was also bumped from 4s to 6s for extra headroom.
- **`/api/import-csv`'s own time budget raised after the same timeout kept happening anyway**: `maxDuration` was set to 60s under the assumption that was the Hobby-plan ceiling — it isn't, Vercel's Fluid Compute (on by default, including on Hobby) allows up to 300s. Even with thinking turned down, a real ~130-row multi-currency statement sometimes still took Gemini past 55s, and the route's own internal timeout (tied to that 60s budget) kept cutting it off first. Raised `maxDuration` to 180s and the Gemini call's own abort timeout to 165s, comfortably inside the platform's real limit.
- **Multi-currency CSV/PDF import fixed**: a real Revolut export (VND/AUD/EUR mixed in one file) imported successfully but with every amount treated as already being in the home currency — no conversion, so the balance was thrown off by orders of magnitude. `/api/import-csv`'s schema now asks Gemini for a per-row ISO 4217 currency code whenever the statement distinguishes one (a "Divisa"/"Currency" column, a changing symbol) — left blank for statements that are just one implicit currency. The client (`economy-section.tsx`) then converts each foreign-currency row to the home currency using the same daily-cached exchange-rate lookup (`lib/exchange-rates.ts`) the manual add-transaction flow already uses, and stores `currency`/`originalAmount` on the transaction so the original figure isn't lost. A currency Gemini reports but that has no exchange rate available that day is left OUT of the import entirely (never guessed) and counted in a new "N sin convertir" notice, rather than risk silently corrupting the balance again.
