...
4. **"Importar CSV" button (Economía, being trialed)**: `POST /api/import-csv`. The user uploads their bank's CSV export as-is (CommBank, Revolut, any bank — any columns, any date format, any decimal/currency style) and the raw file content is sent to Gemini (`GEMINI_API_KEY`, the same key already used by `/api/quick-transaction` for AI category inference — no new key needed; model overridable via `GEMINI_CSV_MODEL`, defaults to `gemini-2.5-flash`), which detects the format and returns normalized transactions (ISO date, signed amount, one of the app's 9 fixed categories) via structured JSON output. The client then drops anything that already matches an existing transaction (same date + amount + description) before inserting the rest via the same batch-insert path as `addTransactions` — so re-uploading the same statement, or one with overlapping dates, doesn't duplicate rows. No preview/review screen — it imports directly. Currently assumes the CSV is already in the user's home currency (no conversion). See the note at the top of `app/api/import-csv/route.ts` for the privacy trade-off this implies, and the Privacy note below.
...
### 11 Sep 2026
- **Presupuestos (category budgets) removed entirely**: the feature never got real usage, so it was pulled to keep the app simpler — `CategoryBudget` type, `budgets` table row type, `fetchBudgets`/`setBudget` in the store, and the whole budgets card in Resumen are gone. `AppData` no longer has a `budgets` field.
