...
export function OverviewSection({
  ...
  const currentWeek = getWeekNumberFromISO(today, weekStartDay)
  ...
  const inPeriod = (dateStr: string) => {
    ...
    if (period === "semanal") return getWeekNumberFromISO(dateStr, weekStartDay) === currentWeek
    return dateStr.startsWith(currentMonth)
  }

  const periodTx = useMemo(() => transactions.filter((t) => inPeriod(t.date)), [transactions, period, today, weekStartDay])
